...
async def get_random():
    async with aiohttp.ClientSession() as session:
        # WORDS_API_URL_RANDOM - у цій змінні буде посилання на ендпоінт з бекенду, який на ньому працює
        async with session.get(WORDS_API_URL_RANDOM) as response:
            return await response.json()
        
async def get_next(pk):
    async with aiohttp.ClientSession() as session:
        # WORDS_API_URL_ALL - у цій змінні буде посилання на ендпоінт з бекенду, який на ньому працює
